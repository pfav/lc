...
namespace lc {
	...
	// Error
	struct Error : virtual public Stringer,
...
// Position
	struct Position
			: virtual public Stringer
	{
		static const int defaultLine   = 1;
		static const int defaultColumn = 0;
		static const int defaultOffset = 0;

		Position(int line = defaultLine, int column = defaultColumn, int offset = defaultOffset)
				: Line{line},
				  Column{column},
				  Offset{offset}
		{}
	...
	struct ScanError : virtual public Error {
		ScanError(const string& message)
				: Error{description(), message}
		{}
		ScanError(const string& filename,
				   const string& message,
				   const Position& pos)
				: Error{description() + " " + filename + ":" + pos.str(),
						message}
		{}
	...
	struct ParseError : virtual public Error {
		ParseError(const string& message)
				: Error{description(), message}
		{}
		ParseError(const string& filename,
				  const string& message,
				  const Position& pos)
				: Error{description() + " " + filename + ":" + pos.str(),
						message}
	...
	struct EvalError : virtual public Error {
		EvalError(const string& message)
				: Error{description(), message}
		{}
		EvalError(const string& filename,
				   const string& message,
				   const Position& pos)
				: Error{description() + " " + filename + ":" + pos.str(),
						message}
			...
			: virtual public Stringer
	{
		...
		Token() = delete;
		Token(Type type, const Position& pos = {})
				: Pos{pos},
				  ValueType{type}
		{}
	...
	public:
		...
		bool operator==(const NameAST& n) const
		{
			return Name == n.Name;
		}
		...
		virtual string describe(const string& prefix) const override
		{
			...
			ss << prefix
			   << "("  << quoted(Name) << ' ' << Index << ' ' << std::boolalpha << Bound << ")";
	...
	public:
		FunctionAST(const string& argument, unique_ptr<AST> body)
				: Argument{argument},
				  Body{move(body)}
		{
			if(!Body) throw ParseError{"Invalid Function"};
		}
			...
			: public AST
	{
		...
		ApplicationAST(unique_ptr<AST> lhs, unique_ptr<AST> rhs)
				: Lhs{move(lhs)},
				  Rhs{move(rhs)}
	  {
	  	if (!Lhs||!Rhs) throw ParseError{"Invalid Application"};
	  }
	...
	public:
		Scanner(istream& input, const string& filename = "<in>", const string &loc = defaultLocale)
				: Input{input},
				  Locale{locale(loc)},
				  Filename{filename},
				  Pos{initialPosition},
				  Ch{},
				  currentToken{Token::undefined, initialPosition},
 ...
 * */
	...
	protected:
		...
		// <application> := <expression><expression>
		unique_ptr<AST> parseApplicationExpr(vector<string> scope, unique_ptr<AST> expr = nullptr) {
			if (expr == nullptr) {
		...
		unique_ptr<AST> parseParenExpr(vector<string> scope) {
			...
			if (scanner.curr() != Token::rparen) {
				throw ParseError(scanner.fileName(), "parser: Expected ')'", scanner.position());
			}
			consume(); // consume ')'
			return expr;
		}
		...
		unique_ptr<AST> parseFunctionExpr(vector<string> scope) {
			...
			if (scanner.curr() != Token::id){
				throw ParseError(scanner.fileName(), "Expected identifier", scanner.position());
			}
		...
		vector<unique_ptr<AST>> parse() {
			...
			while (scanner.curr() != Token::eof) {
				...
				if (scanner.curr() != Token::eof && scanner.curr() != Token::nl) {
					throw ParseError(scanner.fileName(), "Expected newline", scanner.position());
				}
				consume(); // consume '\n'
			}
	...
	class Interpreter {
		...
		bool isName(AST *node)
		{
			return node && dynamic_cast<NameAST*>(node);
		}
		...
		unique_ptr<AST> subst(int n, AST* value, unique_ptr<AST> node)
		{
			...
			if (isName(node.get())){
				...
				dynamic_cast<FunctionAST*>(node.get())->Body = subst(n + 1, value, move(body));
				...
				dynamic_cast<ApplicationAST*>(node.get())->Lhs = subst(n, value, move(lhs));
				dynamic_cast<ApplicationAST*>(node.get())->Rhs = subst(n, value, move(rhs));
			}
		...
		unique_ptr<AST> evalExpr(unique_ptr<AST> ast) {
			...
			while (true) {
				if (isApplication(ast.get())) {
					unique_ptr<ApplicationAST> app{dynamic_cast<ApplicationAST*>(ast.release())};
					...
					if (dynamic_cast<FunctionAST*>(app->lhs())){
						unique_ptr<FunctionAST> lhs{dynamic_cast<FunctionAST*>(app->Lhs.release())};
